...
const defaultActl = async (call, { tenant, sub, scope }, rpcname) => {
  ...
  const plusRules = [`T:${tenant}`, `U:${sub}`, ...scope.map(s => `R:${s}`)];
  const minusRules = [`T:-${tenant}`, `U:-${sub}`, ...scope.map(s => `R:-${s}`)];
  const allow = permissionsStore[tenant][rpcname]
    .rule
    .filter(rule => plusRules.filter(prule => prule === rule).length).length;
  const deny = permissionsStore[tenant][rpcname]
    .rule
    .filter(rule => minusRules.filter(prule => prule === rule).length).length;
  if (deny || !allow) {
    throw new Error(`Access denied to ${rpcname} - ${sub}.${tenant}.${JSON.stringify(scope)}`);
  }
};
...
async function server(ch) {
  ...
  */
  return refToCall => (name, first, second) => {
    ...
    ch.prefetch((process.env.PREFETCH && parseInt(process.env.PREFETCH, 10)) || 1);
    ch.consume(q, (msg) => {
      try {
